...
unsigned long Vocabulary::describeFeatures_TS(const string &DescriptorType) throw(std::runtime_error) {
    ...
    for (; names_iter != names_iter_fal; ++names_iter) {
        bool grayscale(DescriptorType == "OpponentSURF" || DescriptorType == "OpponentSIFT" ||
                       DescriptorType == "OpponentBRIEF");
...
unsigned long
Vocabulary::clusterDescriptors(const string &method, const int &number_cluster) throw(std::runtime_error) {
    ...
    if (method == "kMeansClustering") {
        ...
        cv::kmeans(GreatMatrix, number_cluster, PointToCluster, term_crit, attempts, flags,
                   cv::_OutputArray(ClusterCenters));
    ...
    if (method == "hierarchicalClustering") {
        ...
        if (DescriptorExtractorType == "BRIEF" || DescriptorExtractorType == "BRIEF64") {
            total_words = flann::hierarchicalClustering < cv::flann::L2 < unsigned char > >
                          (GreatMatrix, clCenters, parameters);
        } else {
            total_words =
                    flann::hierarchicalClustering < cv::flann::L2 < float > > (GreatMatrix, clCenters, parameters);
        }
...
void Vocabulary::buildAllIndeces(const string &Index_path, const string &InvIndex_path) throw(runtime_error) {
    ...
    cv::flann::GenericIndex <cv::flann::L2<float>> flannIndex(ClusterCenters,
                                                              flannIndexParams); //(cvflann::IndexParams &)flannIndexParams);
    cvflann::SearchParams knnSearchParams(
            32); //The number of times the tree(s) in the index should be recursively traversed.
    ...
    iter_inv_index = sowInvertIndex.begin(),
...
vector<int> Vocabulary::getSowBowRepr_img(const cv::Mat &current_picture,
                                          const int &method,                        // why do i need a method here??
    ...
    try {
        ...
        grayscale = (DescriptorExtractorType == "SURF" || DescriptorExtractorType == "SURF128" ||
                     DescriptorExtractorType == "SIFT" || DescriptorExtractorType == "BRIEF" ||
                     DescriptorExtractorType == "BRIEF64");
        ...
        } else if ((DescriptorExtractorType == "BRIEF" && dataDimension == 64) ||
                   DescriptorExtractorType == "BRIEF64") {
        ...
        bow_repres = getFlannSowBowVector(flannIndObj, curImgDescriptors, flann_labels_inner, flann_dist_inner,
                                          flann_knn, knnSearchParams, 1);
...
inline string
Vocabulary::generatePath(const string &inp_base, const size_t &number, const string &inp_extension) const {
...
cv::Mat Vocabulary::describeImgFeatures(const cv::Mat &img_gray, vector <cv::KeyPoint> &key_points,
                                        const cv::Ptr <cv::DescriptorExtractor> &descriptorExtractor) const {
    cv::Mat descriptors;
...
vector <cv::KeyPoint>
Vocabulary::getImgFeatures(const cv::Mat &img_gray, const cv::Ptr <cv::FeatureDetector> &detector) const {
...
cv::Mat
Vocabulary::describeImgSurfFeatures(const cv::Mat &img_gray, vector <cv::KeyPoint> &key_points, const int nOctaves,
                                    const int nOctaveLayers, const bool extended) const {
    cv::Mat descriptors;
    cv:
    SURF surfer(500, nOctaves, nOctaveLayers, extended, false);
    surfer.compute(img_gray, key_points, descriptors);
    cout << "describeImgSurfFeatures: computed " << descriptors.rows << " descriptors" << endl;
    return descriptors;
}

cv::Mat Vocabulary::describeImgBriefFeatures(const cv::Mat &img_gray, vector <cv::KeyPoint> &key_points,
                                             const int bits_n) const {
    cv::Mat descriptors;
