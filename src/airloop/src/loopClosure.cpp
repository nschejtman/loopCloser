...
void imageProcessing(const sensor_msgs::ImageConstPtr &msg, flann::GenericIndex <cv::flann::L2<float>> &flannIndObj) {
    ...
    try {
        lc = ClosureDetection.calcProbabCamera(VocabSet, flannIndObj, NOLC_WITH, PROBAB_THRESH, N_NEIGH, GEOM_THRESH,
                                               cv_ptr->image, img_n, timeStamp.str().c_str(), odom.str().c_str());
    }
...
void printUsage() {
    ...
    cout
            << "--loadDBDir=value, optional parameter with mandatory argument, specify the database directory you wish to load. (Not compatible with option --newDBName)"
            << endl << endl;
    cout
            << "--newDBName=value, optional parameter with mandatory argument, specify the name of the new database. (Not compatible with option --loadDBDir)"
            << endl << endl;
    cout
            << "--numberOfSkippedFrame=value, optional parameter with mandatory argument, specify the number of frame to be skipped before performing loop closing."
            << endl << endl;
    cout
            << "--loopClosureThresh=value, optional parameter with mandatory argument, specify the threshold for loop closing."
            << endl << endl;
    cout
            << "--directNeighboors=value, optional parameter with mandatory argument, specify the number of neighbor frames."
            << endl << endl;
    cout
            << "--geomThresh=value, optional parameter with mandatory argument, specify the geometry threshold for loop closing."
            << endl << endl;
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber img_sub = n.subscribe<sensor_msgs::Image>("/images", 100,
                                                              boost::bind(imageProcessing, _1, boost::ref(FlannObj)));
