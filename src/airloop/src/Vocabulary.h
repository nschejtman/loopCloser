...
public:
    ...
    std::vector<int> getSowBowRepr_img(const cv::Mat &current_picture, const int &method,
                                       cv::flann::GenericIndex <cv::flann::L2<float>> &flannIndObj,
                                       std::vector <cv::KeyPoint> &curImgFeatures, cv::Mat &curImgDescriptors,
                                       cv::Mat &flann_labels,
                                       cv::Mat &flann_dist) const;  // <--- overloading  is possible
    std::vector<int> getSowBowRepr_img(cv::Mat &imgDescr, cv::flann::GenericIndex <cv::flann::L2<float>> &flannIndObj,
                                       cv::Mat &flann_labels, cv::Mat &flann_dist) const;
...
private:
    ...
    inline std::string generatePath(const std::string &inp_base, const std::size_t &number,
                                    const std::string &inp_extension) const;           // generate address of the next file to write to
    std::vector <cv::KeyPoint>
    getImgFeatures(const cv::Mat &img_gray, const cv::Ptr <cv::FeatureDetector> &detector) const;

    cv::Mat describeImgFeatures(const cv::Mat &cur_image, std::vector <cv::KeyPoint> &key_points,
    ...
    getFlannSowBowVector(flann::GenericIndex <cvflann::L2<float>> &flannIndex, Mat &descr_query, Mat &indices,
                         Mat &dist, int knn, cvflann::SearchParams params, int sow_bow) const;

    cv::Mat describeImgSurfFeatures(const cv::Mat &img_gray, std::vector <cv::KeyPoint> &key_points, const int number1,
                                    const int number2, const bool number3) const;
