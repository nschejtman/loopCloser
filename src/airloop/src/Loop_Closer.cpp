...
map<int, vector<int> >
Loop_Closer::recalcNewInvIndex(vector<int>::iterator curImgBow_beg, vector<int>::iterator curImgBow_end,
...
vector<float> Loop_Closer::calcImg_tfIdf(vector<int>::const_iterator bow_beg, vector<int>::const_iterator bow_end,
                                         const int n_features_img) const {
...
map<int, float> Loop_Closer::getSimilarityCoefficients(vector<int>::const_iterator bow_iterat,
    ...
    for (; bow_iterat != bow_iterat_fal; ++bow_iterat) {
        if (*bow_iterat != 0) {
            map < int, vector < int > > ::const_iterator
            map_it = newInvIndex.find(vw_number);    // find other documents with this vw
            if (map_it != newInvIndex.end()) {
                vector<int>::const_iterator ii_iter = map_it->second.begin(), ii_iter_fal = map_it->second.end();
                for (; ii_iter !=
                       ii_iter_fal; ++ii_iter) {                                        // for each image comprising this word
                    //	cout<<*ii_iter<<" ";
...
vector<int> Loop_Closer::takeAvgImage(const int vw_n, const int avgImg_rows, int &n_descr_avgImg) const {
    ...
    itr_vw = common_words.begin(),
            itr_vw_fal = common_words.begin() + avgImg_rows;

    if (itr_vw_fal > common_words.end())
        itr_vw_fal = common_words.end();        // in some cases common_words.size() < avgImg_rows
...
map<int, float> Loop_Closer::getLikelihoodCoef(const map<int, float>::const_iterator sim_beg,
                                               const map<int, float>::const_iterator sim_fal) const {
    map<int, float> lkl;
...
Mat Loop_Closer::calcIdfIset(vector<int>::const_iterator bow_beg, const int total_n_vw) const {
    ...
    Mat idf_iset_dev(total_n_vw, 1, CV_32FC1,
                     1);    // a Mat-vector to store w, 1s won't be changed and will turn to 0 after log operation
    Mat idf_log;
    MatIterator_<float> iter_idf_dev = idf_iset_dev.begin<float>(), iter_idf_dev_fal = idf_iset_dev.end<float>();
    int n_run = 0;
    for (; iter_idf_dev !=
           iter_idf_dev_fal; ++iter_idf_dev, ++bow_beg) {                // for each position... (total number is equal to total number of vw)
        if (*bow_beg != 0) {        // we wont need idf for words, idf of which  is equal to 0
            float n_img_with_vw = 0;
            map < int, vector < int > > ::const_iterator
            iter_idf_cur = newInvIndex.find(n_run);                // is the word already in the inverted index?
...
Loop_Closer::calcProbabCamera(Vocabulary &VocabularyObject, flann::GenericIndex <cv::flann::L2<float>> &flannIndObj,
                              const unsigned neighbor, const float lc_threshold, const unsigned subset_range,
                              const float gmtr_threshold, Mat &img_current, unsigned int img_n, string ts,
                              string odom) throw(runtime_error) {
    ...
    vector<int> curImg_bow = VocabularyObject.getSowBowRepr_img(img_current, 0, flannIndObj, curImgFeatures,
                                                                curImgDescriptors, curImgFlannLabels, curImgFlannDist);
...
//	cout<<"Starting new Inverted Index Computing..."<<endl;
    ...
    if (img_n > neighbor) {
        ...
        if (img_n < 100 || (img_n - neighbor) % 100 == 0) {                // is it enough??
            ...
            ii_map_itr = newInvIndex.begin(), ii_map_itr_fal = newInvIndex.end();
            for (; ii_map_itr != ii_map_itr_fal; ++ii_map_itr) {                                //for each key
                ...
                for (; inner_runner !=
                       inner_runner_fal; ++inner_runner) {                        // for each element of the member vector
                    if ((*inner_runner) == -1)
        ...
        //******** calc. similarity map ******************************
        map<int, float> sim_cfs = getSimilarityCoefficients(curImg_bow.begin(), curImg_bow.end(), curImg_w_log.begin(),
                                                            curImg_w_log.end(), neighbor, img_n);
        ...
        for (int to_pos = -1; to_pos != fal_not_neighbor; ++to_pos) {
            ...
            if (to_pos == -1) {
                p_lclosing_with_to_pos = 0.9 * closureProbab[img_n - 1].at(to_pos + 1) +
                                         0.1 * (1 - closureProbab[img_n - 1].at(to_pos + 1));
            } else {
...
//		cout<<"   +++p_lclosing: "<<p_lclosing<<endl;
            ...
            if (lc_iter != curImg_pos_probab.begin()) {
                ...
                *lc_iter = *lc_iter / p_lclosing;
                if (sum >= lc_threshold) {
                    ...
                    int simImg = position_number - subset.size() / 2 +
                                 1;        // some trouble is possible in the very beginning
        ...
        if (probableClosureCandidates.size() != 0) {
            ...
            vector < pair < int, float > > ::const_iterator
            candidate_it = probableClosureCandidates.begin(), candidate_it_fal = probableClosureCandidates.end();
            for (; candidate_it != candidate_it_fal; ++candidate_it) {
                ...
                kpMatch_it = compatPoints.begin(),
                        kpMatch_it_fal = compatPoints.end();

                vector < vector < KeyPoint > > ::const_iterator
                vvp_iter_img_n = newKeyPointsByImage.begin() + img_n,
                ...
                findFundamentalMat(points1, points2, status, FM_RANSAC, 3.,
                                   0.9);            // what corresponds to '3' method&
                ...
                if (ret.second) {
                    std::ofstream outFile((outputFolder + closureProbabFile).c_str(),
                                          std::ios::out | std::ifstream::app);
                    outFile << img_n << " ";
    ...
    for (int i = 0; i < detectedLoopClosures.size(); i++) {
        ...
        outFileClosure << detectedLoopClosures[i][0] << "    " << first.first << "    " << first.second << "    "
                       << detectedLoopClosures[i][1] << "    " << second.first << "    " << second.second << "    "
                       << detectedLoopClosures[i][2] << "    " << detectedLoopClosures[i][3];
        outFileClosure << endl;
    }
    outFileClosure.close();

    std::ofstream outFileInvInd((outputFolder + invIndFile).c_str(), std::ios::out | std::ifstream::out);
    for (std::map < int, std::vector < int > > ::iterator it = newInvIndex.begin(); it != newInvIndex.end();
    it++){
