...
public:
    ...
    calcProbabCamera(Vocabulary &VocabularyObject, flann::GenericIndex <flann::L2<float>> &flannIndObj,
                     const unsigned neighbour, const float lc_threshold, const unsigned subset_range,
                     const float gmtr_threshold, Mat &image, unsigned int img_n, string ts,
...
private:
    ...
    recalcNewInvIndex(std::vector<int>::iterator curImgBow_beg, std::vector<int>::iterator curImgBow_end,
                      int img_number) const;

    std::vector<float> calcImg_tfIdf(std::vector<int>::const_iterator bow_beg, std::vector<int>::const_iterator bow_end,
                                     const int n_features_img) const;

    std::map<int, float> getSimilarityCoefficients(std::vector<int>::const_iterator sow_iterat,
                                                   std::vector<int>::const_iterator sow_iterat_fal,
                                                   std::vector<float>::const_iterator itr_curImg_w_log,
                                                   std::vector<float>::const_iterator itr_curImg_w_log_fal,
                                                   int neighbor, int img_n) const;

    std::vector<int> takeAvgImage(const int vw_n, const int avgImg_rows, int &n_descr_avgImg) const;

    std::map<int, float> getLikelihoodCoef(const std::map<int, float>::const_iterator sim_beg,
                                           const std::map<int, float>::const_iterator sim_end) const;
